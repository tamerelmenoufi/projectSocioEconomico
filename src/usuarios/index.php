              ...
              <tbody>
                <?php
                  $query = "select 
                            ...
                            where a.deletado != '1' {$where} ".({$_SESSION['ProjectSeLogin']->codigo} = 1:{$_SESSION['ProjectSeLogin']->codigo} = 1?(($_SESSION['ProjectSeLogin']->perfil == 'crd')?" and a.coordenador = '{$_SESSION['ProjectSeLogin']->codigo}' ":false).(($_SESSION['ProjectSeLogin']->perfil == 'adm')?" and (a.perfil != 'adm' or a.codigo = '{$_SESSION['ProjectSeLogin']->codigo}') ":false))." order by a.nome asc";
