    ...
    <form id="form-<?= $md5 ?>">
        <div class="row">
            <div class="col">
                ...
                <?php
                if($d->codigo != 1 and ($_SESSION['ProjectSeLogin']->codigo != $d->codigo) or !$_POST['cod']){
                ?>
                ...
                <div class="form-floating mb-3">
                    <select name="coordenador" id="coordenador" class="form-control" placeholder="Coordenador">
                        ...
                        ?>
                        <option <?=(($d->perfil != 'usr' and $_SESSION['ProjectSeLogin']->perfil != 'crd')?'disabled':false)?> value="<?=$s->codigo?>" <?=(($d->coordenador == $s->codigo)?'selected':false)?>>
                            <?=$s->nome?>
                        </option>
                        <?php
                            }
