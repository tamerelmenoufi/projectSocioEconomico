<?php
    ...
    $query_geral = "select * from se where acao = '0' /*and coordenadas != ''*/ group by municipio, local, bairro_comunidade order by municipio desc limit 1";
    ...
    while($d_geral = mysqli_fetch_object($result_geral)){
        ...
        $query = "select
            ...
            (select count(*) from se where municipio = '{$d_geral->municipio}' and beneficiario_encontrado = 'Não' and local = '{$d_geral->local}' and bairro_comunidade = '{$d_geral->bairro_comunidade}') as nao_encontrado
        ";
        $result = mysqli_query($con, $query);
